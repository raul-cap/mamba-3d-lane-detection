...
class LATR(nn.Module):
    def __init__(self, args):
        ...
        self.encoder = AutoModel.from_pretrained("nvidia/MambaVision-S-1K", trust_remote_code=True)
    ...
    def forward(self, image, _M_inv=None, is_training=True, extra_dict=None):
        avg_pool, out_featList = self.encoder(image)

        fpn_input = [
            out_featList[1],  # Corresponds to the 512-channel output
            out_featList[2],  # Corresponds to the 1024-channel output
            out_featList[3],  # Corresponds to the 2048-channel output
        ]
        neck_out = self.neck(fpn_input)
