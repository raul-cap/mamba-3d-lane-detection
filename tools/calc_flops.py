...
parser = argparse.ArgumentParser(description='FLOPs calculator for LATR')
...
if hasattr(cfg, 'K'):
    cfg.K = np.array(cfg.K)
cfg.anchor_y_steps = np.linspace(3, 103, 20)
if hasattr(cfg, 'anchor_y_steps_dense'):
    cfg.anchor_y_steps_dense = np.array(cfg.anchor_y_steps_dense)
else:
    cfg.anchor_y_steps_dense = np.linspace(3, 103, 200)
...
flops = FlopCountAnalysis(model, (input_tensor, None, False, extra_dict))
