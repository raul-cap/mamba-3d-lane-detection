...
# from mmdet.optim import build_optimizer
...
class Runner:
    ...
    def build_custom_optimizer(self, model, optimizer_cfg):
        param_groups = []
        custom_keys = optimizer_cfg.get('paramwise_cfg', {}).get('custom_keys', {})
        
        # Iterează prin parametrii modelului și aplică regulile specifice din custom_keys
        for name, param in model.named_parameters():
            if not param.requires_grad:
                continue
            applied = False
            for key, setting in custom_keys.items():
                if key in name:
                    param_groups.append({
                        'params': [param],
                        'lr': optimizer_cfg['lr'] * setting.get('lr_mult', 1.0),
        ...
        # Creează optimizerul AdamW
        optimizer = torch.optim.AdamW(param_groups, lr=optimizer_cfg['lr'], weight_decay=optimizer_cfg['weight_decay'])
        return optimizer
    ...
    def _get_model_ddp(self):
        ...
        #     model,
        #     args.optimizer_cfg)
        optimizer = self.build_custom_optimizer(model, args.optimizer_cfg)
    ...
    def resume_model(self, model, path=''):
        ...
        if len(path) == 0 and args.resume:
            ...
            if not os.path.isfile(path):
                print("RESUMING FROM LAST BEST EPOCH")
